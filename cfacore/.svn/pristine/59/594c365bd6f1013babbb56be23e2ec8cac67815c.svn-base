        ...
        public object AfterReceiveRequest(ref Message request, IClientChannel channel, InstanceContext instanceContext)
        {
            if (request.Properties.ContainsKey("UriTemplateMatchResults"))
            {
                ...
                if(string.IsNullOrEmpty(format))
                    format = match.QueryParameters["format"];
                if ("json".Equals(format, StringComparison.InvariantCultureIgnoreCase) || "jsonp".Equals(format, StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    if (!string.IsNullOrEmpty(callback))
                    {
                        ...
                        callback = match.QueryParameters["callback"];
                        if (!string.IsNullOrEmpty(callback))
                        {
                            match.QueryParameters.Remove("callback");
                            return callback;
